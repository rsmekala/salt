...
def shutdown(**kwargs):
    ...
    try:
        ...
        # TODO: Let PyEZ decide what to honour in_min or at
        if 'in_min' in op:
...
def install_config(path=None, **kwargs):
    ...
    format
        Determines the format of the contents. If not provided, format is assumed to
        be the same as file extension.
    ...
    .. note::
        If you are facing problems to render the template, please refer
        https://github.com/saltstack/salt/issues/54096
    ...
    try:
        # This is a necessary temp file creation as get_template module requires
        # a destination. Please note the generated temp file will not have any extension
        template_cached_path = salt.utils.files.mkstemp()
        # Render the template into the template_cached_path. If you are facing any issue with rendering,
        # check https://github.com/saltstack/salt/issues/54096
    ...
    if not os.path.isfile(template_cached_path):
        ret['message'] = 'Invalid template file path generated by salt.'
        ret['out'] = False
        return ret
    ...
    # op is passed as a dictionary to load method, so relevant
    # variables need to be set in op dictionary itself
    ...
    # This logic may be a little confusing, but in case you are wondering
    # wondering about the default merge method, it is `replace`. If no option is
    # provided PyEZ will use `merge`
